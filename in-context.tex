...
\subsection[Ustawienia]{Definicje, konfiguracje i parametry}

W \ConTeXt'cie konsekwentnie stosowany jest trójstopniowy schemat określania działania
używanych poleceń. Składa się on z definicji, konfiguracji i modyfikacji działania polecenia przy 
pomocy parametrów. Łatwo zapamiętać stosowaną konwencję, definicje rozpoczynają się od przedrostka \type{\define}, konfiguracja 
działania zdefiniowanego wcześniej polecenia od \type{\setup}, natomiast parametry są wywoływane (zwykle czy zawsze?) przez dodanie do polecenia ich listy w nawiasie kwadratowym po poleceniu. Zwykle  (zawsze?) te same parametry są dostępne na każdym etapie. 
...
\subsection[Liczniki]{Liczniki}

Większość czynności dokonywanych przez \ConTeXt podczas przetworzania
dokumentu łączy się z używaniem zdefiniowanych w pakiecie liczników,
które zapisują wystąpienia różnych elementów w dokumencie. Można
używać aktualnych wartości liczników we własnych makrach lub
umieszczać je w składanym dokumencie, można też je modyfikować, aby
zmienić działanie \ConTeXt-u lub wartość, jaka ma się pojawić w składzie. 

[Liczniki zdefiniowane w ConteXt-cie]

Własny licznik definiuje się przy pomocy polecenia
\type{\definenumber}, a jego wartość zwiększa się przy pomocy
\type{\incrementnumber}. 

\startbuffer[liczniki]
\definenumber[one]
\incrementnumber[two]
\stopbuffer
...
\useURL[liczniki][http://wiki.contextgarden.net/Counters]
\from[liczniki]
...
\section[Style akapitów]{Style akapitów}

Akapit, czyli (dla \ConTeXt'u) fragment tekstu pomiędzy dwoma pustymi liniami lub  poleceniem \type{Przygotowując \par} a następnymi
dwoma pustymi liniami lub polecenia \type{\par}, zwykle powinien posiadać jednolity styl od początku do 
końca książki, zwłaszcza w przypadku powieści. Jego parametry powinno
się ustalić na początku pliku źródłowego i  nie zmieniać później,
jeżeli nie ma się zamiaru pozbawić składanego tekstu jednolitej
formy.  Istnieje szereg przypadków, kiedy różne style akapitów są
uzasadnione ich funkcją w strukturze dokumentu, na przykład w
podręcznikach. 
